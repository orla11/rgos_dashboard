<template>
  <div id="data-list-list-view" class="data-list-container">
    ...
    <vs-table ref="table"  pagination :max-items="itemsPerPage" search :data="groups">
        ...
        <div class="flex flex-wrap-reverse items-center data-list-btn-container">
          ...
          <div class="btn-add-new p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-primary border border-solid border-primary"
            @click="toNew">
              ...
              <span class="ml-2 text-base text-primary">Add New Group</span>
          </div>
        </div>
      ...
      <template slot="thead">
        <vs-th sort-key="name">Group Name</vs-th>
          ...
          <tbody>
            <vs-tr :key="data[index]._id" v-for="(tr, index) in data">
              ...
              <vs-td class="whitespace-no-wrap">
                <feather-icon icon="EditIcon" svgClasses="w-5 h-5 hover:text-primary stroke-current" @click.stop="editData(data[index]._id)" style="cursor: pointer;"/>
                <feather-icon icon="TrashIcon" svgClasses="w-5 h-5 hover:text-danger stroke-current" class="ml-2" @click.stop="deleteData(data[index]._id)" style="cursor: pointer;"/>
              </vs-td>
...
export default {
    ...
    methods: {
      ...
      deleteData (id) {
        this.$store.dispatch('group/deleteGroup',id).then(() => {
            this.$vs.notify({
              title:'Group deleted',
              text:'This group was successfully deleted',
              color:'success',
              position:'top-right',
              icon:'icon-check',
              iconPack:'feather',
      ...
      toNew(){
        this.$router.push({name: 'group-new'})
      }
    },
    mounted () {
      this.isMounted = true
